...
void Window::movePlayer() {
	...
	if (keys.a == 1 && keys.m == 1) player.lookAngle -= 1 * deltaTime;
...
void Window::key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
	...
	if (action == GLFW_PRESS) {
		if (key == GLFW_KEY_W) win->keys.w = 1;
		if (key == GLFW_KEY_S) win->keys.s = 1;
		if (key == GLFW_KEY_A) win->keys.a = 1;
		if (key == GLFW_KEY_D) win->keys.d = 1;
		if (key == GLFW_KEY_M) win->keys.m = 1;
		if (key == GLFW_KEY_COMMA) win->keys.sr = 1;
		if (key == GLFW_KEY_PERIOD) win->keys.sl = 1;
	} else if (action == GLFW_RELEASE) {
		if (key == GLFW_KEY_W) win->keys.w = 0;
		if (key == GLFW_KEY_S) win->keys.s = 0;
		if (key == GLFW_KEY_A) win->keys.a = 0;
		if (key == GLFW_KEY_D) win->keys.d = 0;
		if (key == GLFW_KEY_M) win->keys.m = 0;
		if (key == GLFW_KEY_COMMA) win->keys.sr = 0;
		if (key == GLFW_KEY_PERIOD) win->keys.sl = 0;
	}
}
